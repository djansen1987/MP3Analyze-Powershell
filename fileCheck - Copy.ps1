...
function Get-Response($Name){
    ...
    $DumpButton.Location = New-Object System.Drawing.Point(225,120)
    $DumpButton.Size = New-Object System.Drawing.Size(75,23)
    $DumpButton.Text = 'Dump'
    $DumpButton.DialogResult = [System.Windows.Forms.DialogResult]::Ignore

    $form.AcceptButton = $DumpButton
    $form.Controls.Add($DumpButton)
    ...
    $textBox1.Top = 235;
    $textBox1.width = 20;
    $textBox1.Text = "$($global:CheckTime)";

    #$form.Add_Shown({$form.Activate(); $textBox1.focus()})
    $form.Controls.Add($textLabel1);
    $form.Controls.Add($textBox1);

    $global:CheckTime = $textBox1.Text
    $form.Topmost = $true
...
function Check-File($File){
    ...
    if($response -eq "Yes"){
        New-Item -ItemType Directory ($Destdir + "\Goed\") -Force -ea SilentlyContinue|Out-Null
        Move-Item -Path $_.Fullname -Destination ($Destdir + "\Goed\")

        return "Goed"
    }elseif($response -eq "OK"){
        New-Item -ItemType Directory ($Destdir + "\Top\") -Force -ea SilentlyContinue|Out-Null
        Move-Item -Path $_.Fullname -Destination ($Destdir + "\Top\")
        return "Top"
    }elseif($response -eq "Ignore"){
        
        New-Item -ItemType Directory ($Destdir + "\Dump\") -Force -ea SilentlyContinue|Out-Null
        Move-Item -Path $File.Fullname -Destination ($Destdir + "\Dump\")
        return "Dump"
    }elseif($response -eq "No"){
        
        New-Item -ItemType Directory ($Destdir + "\Bad\") -Force -ea SilentlyContinue|Out-Null
        Move-Item -Path $File.Fullname -Destination ($Destdir + "\Bad\")
        return "Dump"
...
$org = Read-Host "Y= Original - N= temp\done"
if($org -eq "y"){
    $Global:Destdir = $Filepath
}else{
    $Global:Destdir = "c:\temp\#MP3-Done$(get-date -Format ddMMyyHHmm)"
